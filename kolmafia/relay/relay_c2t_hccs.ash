...
void main() {
	...
	string [string] ascension = {
		"c2t_hccs_AscendClass":"Ascension Class? 1:SC,2:TT,3:PM,4:SA,5:DB,6:AT",
		"c2t_hccs_WhatCore":"Which Core of Ascension? 2:Soft 3:Hard",
		"c2t_hccs_ascgender":"Which gender? 1:Male 2:Female",
		};
	...
	if (POST.count() > 1) {
		...
		if (!postError) {
			...
			foreach name,desc in disableable
				set_property(name,POST[name]=="on"?"true":"false");
			foreach name,desc in ascension
				set_property(name,POST[name]);
	...
	//Ascension
	c2t_hccs_writeText("h2","Ascension");
	c2t_hccs_writeText("p","These are for Ascension Script");
	write("<table>");
	c2t_hccs_writeTh("setting","value","description");
	write("<tbody>");
	foreach name,desc in ascension
